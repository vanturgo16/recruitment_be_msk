    ...
    <div class="modal fade" id="edit{{ $data->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-top modal-md" role="document">
            <div class="modal-content">
                ...
                <form class="formLoad" action="{{ route('department.update', encrypt($data->id)) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="col-lg-12 mb-3">
                                ...
                                <select class="form-control select2" name="id_div" required>
                                    ...
                                    @foreach($listDivisions as $item)
                                        <option value="{{ $item->id }}" @if($data->id_div == $item->id) selected="selected" @endif>{{ $item->div_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-lg-12 mb-3">
                                <label class="form-label">Department Name</label><label style="color: darkred">*</label>
                                <input class="form-control" name="dept_name" type="text" value="{{ $data->dept_name }}" placeholder="Input Department Name.." required>
                            </div>
                            <div class="col-lg-12 mb-3">
