...
<div class="page-content">
    <div class="card">
        <div class="card-header">
            <h4 class="text-bold">Step 5 - Signing Schedules</h4>
        </div>
        <div class="card-body">
            @if(session('success'))
            ...
            <div class="table-responsive">
                <table class="table table-bordered" id="signingTable">
                    <thead>
                        <tr>
                            <th>No</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="viewDetailModalLabel{{ $schedule->id }}">Signing Detail</h5>
                                ...
                                <br>
                                <!-- jika status Signing bukan 0 -->
                                @if ($schedule->sign_status != '0')
                                    ...
                                    <div class="modal fade" id="resulttestModal{{ $schedule->id }}" tabindex="-1" aria-labelledby="resulttestModalLabel{{ $schedule->id }}" aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="resulttestModalLabel{{ $schedule->id }}">Signing Detail</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <div class="mb-2">
                                                    ...
                                                    <div class="mb-2">
                                                        <strong>Approval Signing By:</strong><br>
                                                    ...
                                                    <div class="mb-2">
                                                        <strong>Result Signing:</strong><br>
                                                        @if ($schedule->sign_status == '1')
                                                            <span class="badge bg-success">PASSED</span>
                                                        @elseif ($schedule->sign_status == '2')
                            ...
                            <td>
                                <div class="dropdown">
                                    @if ($isAdminHR && $schedule->sign_status == '0')
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton{{ $schedule->id }}">
                                        ...
                                        <li>
                                            <form action="{{ route('signing_schedule.delete', $schedule->id) }}" method="POST" onsubmit="return confirm('Delete this schedule?')">
                                                @csrf
                                                <button type="submit" class="dropdown-item text-danger">Delete</button>
                                            </form>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#resultModal{{ $schedule->id }}">Signing Result</a>
                                        </li>
                                    </ul>
                                    @endif
                                    @if ($schedule->sign_status == '1')
                        ...
                        <div class="modal fade" id="editModal{{ $schedule->id }}" tabindex="-1" aria-labelledby="editModalLabel{{ $schedule->id }}" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <form action="{{ route('signing_schedule.update', $schedule->id) }}" method="POST">
                                        @csrf
                                        <div class="modal-header">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="mb-3">
                                                <label for="sign_date{{ $schedule->id }}" class="form-label">Testing Date</label>
                                                <input type="datetime-local" name="sign_date" id="sign_date{{ $schedule->id }}" class="form-control" value="{{ date('Y-m-d\TH:i', strtotime($schedule->sign_date)) }}" required>
                                            </div>
                                            <div class="mb-3">
                                                <label for="sign_address{{ $schedule->id }}" class="form-label">Testing Address</label>
                                                <input type="text" name="sign_address" id="sign_address{{ $schedule->id }}" class="form-control" value="{{ $schedule->sign_address }}" required>
                                            </div>
                                            <div class="mb-3">
                                                <label for="sign_notes{{ $schedule->id }}" class="form-label">Testing Notes</label>
                                                <textarea name="sign_notes" id="sign_notes{{ $schedule->id }}" class="form-control">{{ $schedule->sign_notes }}</textarea>
                                            </div>
                                        </div>
                                        <div class="modal-footer">
                        ...
                        <div class="modal fade" id="resultModal{{ $schedule->id }}" tabindex="-1" aria-labelledby="resultModalLabel{{ $schedule->id }}" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <form action="{{ route('signing_schedule.update.result', $schedule->id) }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="resultModalLabel{{ $schedule->id }}">Input Signing Result</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            <div class="mb-3">
                        ...
                        <div class="modal fade" id="submitHiredModal{{ $schedule->id }}" tabindex="-1" aria-labelledby="submitHiredModal{{ $schedule->id }}" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <form action="{{ route('signing_schedule.submitHired', encrypt($schedule->id)) }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        <div class="modal-header">
